...
(function(undefined){
    ...
    /************lazy load img***************/ 
	JS.loadImg = function(){
		if(!window.loadImgStatus){
			$(document).bind("scroll", function(){
				window.loadImgStatus = true;
				JS.loadImg();
			});
		}
	    window.lazyImg = $(document).find("img[lazysrc]");
	    if(window.lazyImg.length){
	    	if(window.loadImgTimeout){
	    		clearTimeout(window.loadImgTimeout);
	    	}
	    	window.loadImgTimeout = setTimeout(function(){
	            var clientHeight = document.documentElement.clientHeight;
	            var scrollTop = document.body.scrollTop;
	            $(window.lazyImg).each(function(obj){
	                if((obj.offsetTop + obj.height) >= scrollTop && (obj.offsetTop + obj.height) <= (clientHeight + scrollTop)){
	                    var attrval = obj.getAttribute("lazysrc");
	                    if(attrval){ 
	                        obj.setAttribute("src", attrval);
   ...
   /************AJAX***************/
	...
	JS.getJSONP = function(url, callback) {
		var call = 'call' + new Date().getTime();
		if(url.indexOf("?") > 0){
			url = url + "&callback=" + call;
		}else{
			url = url + "?callback=" + call;
		}
		//将函数名设置为window的一个方法,这样此方法就是全局的了.
		window[call] = window[call] || function( result ) {
			if(result.status == 200){
				callback(result.data);
			}else{
				alert("内部错误");
